...
type DownloaderWorker interface {
	InsertJob(target string, filename string, collection string) (err error)
	Executor(id int)
	StopWorker()
}
...
func (dw *downloaderWorker) InsertJob(target string, filename string, collection string) (err error) {
	if target == "" {
...
func (dw *downloaderWorker) Executor(id int) {
	...
	for job := range dw.jobQueue {
		...
		if _, err := io.Copy(fWriter, res.Body); err != nil {
